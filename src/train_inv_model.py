...
def train(model, trainset, n_epochs, print_every_n_batches=100, batch_size=64,
    ...
    for epoch in range(n_epochs):
        ...
        if validation_set is not None:
            metrics = compute_metrics(model, validation_set, print_metrics=False) 
...
def compute_metrics(model, validation_set, print_metrics=True):
    ...
    for i, c in enumerate(class_names):
        ...
        metrics['cnt/' + c] = class_total[i]
    metrics['accuracy/avg'] = float(np.sum(class_correct)) / np.sum(class_total)
    metrics['loss/validation'] = cross_entropy_loss / np.sum(class_total)
    if print_metrics:
        for k, v in metrics.items():
            print(k, ':', v)
    return metrics
...
def main():
    ...
    n_logits = len(validation_set.get_class_names())
    try:
        model_cls = MODEL_NAME_TO_CLASS_MAP[args.model_name.lower()]
    except:
        print('Unrecognized model name.')
        return
    model = model_cls(n_logits=n_logits)
